...
class OptimistAgent():
    def __init__(self):
        self.agent = Agent(
            ...
            instructions=AgentPrompts.OPTIMIST_INSTRUCTIONS,
            markdown=True,
        )
...
if __name__ == "__main__":
    ...
    try:
        response = optimist_agent.run("The future of renewable energy", stream=False)
        if response is not None:
            # Access the content attribute from the RunOutput object
            print(getattr(response, 'content', str(response)))

    except Exception as e:
        print(f"Error occurred in OptimistAgent: {e}")